...
class AssettoCorsaUdpService extends EventEmitter {
  ...
  startUdpListener(port) {
    try {
      // Check if socket is already bound - safely
      try {
        const address = this.udpSocket.address();
        if (address && address.port) {
          console.log(`UDP socket already bound to port ${address.port}, closing first`);
        ...
        // Socket might not be bound or in a bad state, that's ok
        console.log('UDP socket not bound or in bad state, creating new one');
        this.udpSocket = dgram.createSocket('udp4');
        this.setupUdpSocket();
      }
      
      // Attach an error handler specifically for bind errors to avoid
      // uncaught exceptions crashing the process in hosted environments.
      this.udpSocket.once('error', (err) => {
        if (err && err.code === 'EADDRINUSE') {
          console.warn(`UDP port ${port} already in use, skipping bind:`, err.message);
          // Keep service running but mark as not connected.
          this.isConnected = false;
          this.emit('connection-status', 'disconnected');
          return;
        }
        // For other errors, log them (setupUdpSocket already logs errors too).
      ...
      try {
        ...
        console.error('Failed to create new UDP socket:', retryError);
      }
    }
  }
  ...
  broadcastUpdate() {
    ...
    this.clients.forEach(client => {
      // readyState 1 = OPEN in WebSocket
