...
try {
  if (assettoCorsaUdpService && typeof assettoCorsaUdpService.setupWithHttpServer === 'function') {
    assettoCorsaUdpService.setupWithHttpServer(server);
  }
} catch (err) {
  console.error('Failed to setup Assetto Corsa UDP service with HTTP server:', err);
}

// Start UDP listener on default port (9600)
// Note: This will be reconfigured when race data is loaded
try {
  if (assettoCorsaUdpService && typeof assettoCorsaUdpService.startUdpListener === 'function') {
    assettoCorsaUdpService.startUdpListener(9600);
  }
} catch (err) {
  console.error('Failed to start Assetto Corsa UDP listener:', err);
}
